...
class BertSelfAttention(nn.Module):
  ...
  def transform(self, x, linear_layer):
    # 通过线性层, 把hidden_state变成key, value, query,就是QKV矩阵乘上输入
    ...
    # 应该就是在这一步, 把hidden_size分成了num_attention_heads份,也即是注意力分头的操作
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # 上面三个函数其实就是XW+b的操作, 就是input_ids乘上三个矩阵, 然后加上bias,三个矩阵分别是Q,K,V
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    #TODO 这里可能有问题,按照hint 是先处理output,然后再加上input
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    temp = inputs_embeds + pos_embeds + tk_type_embeds
    temp = self.embed_layer_norm(temp)
    temp = self.embed_dropout(temp)
    return temp
